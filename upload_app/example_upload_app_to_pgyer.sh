...
###
 ...
 # @LastEditTime: 2023-09-25 17:20:43
...
log_title "上传ipa到蒲公英"
sh ${CurrentDIR_Script_Absolute}/upload_app_to_pgyer.sh -f "${ipa_file_path}" -k "${pgyer_api_key_VALUE}" -c "${buildChannelShortcut}" -d "${UpdateDescription}" --should-upload-fast "${ShouldUploadFast}"
if [ $? != 0 ]; then
    # echo "${RED}${errorMessage}${NC}"
    exit 1
fi
echo "${GREEN}上传ipa到蒲公英成功.${NC}"
