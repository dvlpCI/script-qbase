...
###
 ...
 # @Description: 通过人工交互方式获取指定环境变量的值(方式 ①从文件中选择[如果有传文件的话]或者 ②从终端输入）。常用于获取后，执行 env_var_2add_or_update.sh
# @FilePath: sh env_variables/env_var_1get_by_manual.sh --env-name QTOOL_DEAL_PROJECT_PARAMS_FILE_PATH --env-keys-file /Users/qian/Project/Github/script-qbase/example_env_keys_menu.json
### 
...
while [ $# -gt 0 ]; do
    case "$1" in
        --env-name)
            if [ -z "$2" ] || [[ "$2" =~ ^- ]]; then
                log_color_info "错误: --env-name 需要指定一个值"
                exit 1
            fi
            ENV_NAME="$2"
            shift 2
...
if [ -n "${CHOOSE_FROM_ENV_KEYS_FILE_PATH}" ] && [ ! -f "${CHOOSE_FROM_ENV_KEYS_FILE_PATH}" ]; then
    log_color_info "错误: --env-keys-file 的参数值不是有效文件: ${CHOOSE_FROM_ENV_KEYS_FILE_PATH}"
    exit 1
fi
...
showCustomMenuJsonExample() {
    ...
    log_color_info "${BLUE}$(cat ${ENV_KEYS_JSON_FILE_EXAMPLE})${NC}"
}
...
validate_envs_choices_json() {
    ...
    # 检查文件是否存在
    if [ ! -f "$file_path" ]; then
        log_color_info "${RED} ${file_path} 文件不存在，请检查。${NC}\n"
        return 1
    fi
    ...
    if [ $? != 0 ]; then
        log_color_info "${RED}${file_path} 不是有效的 JSON 格式，请检查。${NC}\n"
        return 1
    fi
    ...
    if [ "${has_envs_choices}" != "true" ]; then
        log_color_info "${RED}${file_path} 中缺少 .envs_choices 字段或不是数组格式，请检查。${NC}\n"
        return 1
    fi
    ...
    if [ "${envs_choices_count}" -eq 0 ]; then
        log_color_info "${RED}${file_path} 中的 .envs_choices 数组为空，请添加环境变量配置后再试。${NC}\n"
        return 1
    fi
    ...
    for ((i = 0; i < envs_choices_count; i++)); do
        ...
        if [ -z "${env_key}" ] || [ "${env_key}" == "null" ]; then
            log_color_info "${RED}${file_path} 格式错误：第 $((i+1)) 条缺少 env_key 字段${NC}\n"
            return 1
        fi
        ...
        if [ "${has_env_choices}" == "true" ]; then
            ...
            if [ "${is_array}" != "true" ]; then
                log_color_info "${RED}${file_path} 格式错误：env_key '${env_key}' 的 .env_choices 不是数组格式${NC}\n"
                return 1
            fi
            ...
            if [ "${env_choices_count}" -eq 0 ]; then
                log_color_info "${YELLOW}警告：${file_path} 中 env_key '${env_key}' 的 .env_choices 数组为空${NC}\n"
            fi
        fi
    done

    log_color_info "${GREEN}✓ ${file_path} JSON 格式验证通过，符合环境变量菜单结构。${NC}\n"
}
    ...
    if [ -z "$target_env_key" ]; then
        ...
        while true; do
            ...
            if [ -z "${current_value}" ]; then
                log_color_info "您还未选择环境变量"
            else
                log_color_info "当前环境变量 ${ENV_NAME} 的值为: ${YELLOW}${current_value}${NC}"
            fi
...
showEnvKeyList() {
    ...
    log_color_info "支持的环境变量列表： (详见: ${YELLOW}${json_file}${NC})\n"
    ...
    for ((i = 0; i < ${envKeysCount}; i++)); do
        ...
        log_color_info2 "${GREEN}%-2s${NC}) ${BLUE}%-40s${NC} - ${YELLOW}%s${NC}\n" "${iEnvKeyOptionId}" "${iEnvKey}" "${iEnvDesc}"
    done
}
...
showEnvChoices() {
    ...
    # log_color_info "选中的环境变量的选项信息: ${envConfig}"

    if [ -z "$envConfig" ] || [ "$envConfig" = "null" ]; then
        log_color_info "错误: 未找到 env_key '${target_env_key}' 的配置"
        return 1
    fi
    ...
    if [ "$hasChoices" = "true" ]; then
        ...
        if [ "$choicesCount" -eq 0 ]; then
            log_color_info "该环境变量没有可选的配置项"
            return 1
        fi
        
        log_color_info "请选择 ${BLUE}${target_env_key}${NC} 的值:\n"
...
status_type="env_val_get_success"
message="环境变量信息获取成功"
printf '%s\n' "{
  \"status_type\": \"${status_type}\",
  \"message\": \"${message}\",
  \"response\": {
    \"env_key\": \"${selected_env_key}\",
    \"env_value\": \"${selected_value}\"
  }
}"